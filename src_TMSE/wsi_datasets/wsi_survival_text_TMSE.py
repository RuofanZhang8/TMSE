...
class WSIOmicsSurvivalDataset(WSISurvivalDataset):
    def __init__(self,
        ...
        super().__init__(df_histo, data_source, target_transform, sample_col, slide_col,
                       ...
                       lazy_init=True)
        self.textemb_path = kwargs.get('textemb_path', None)
        self.read_feats = kwargs['read_feats']
    ...
    def __getitem__(self, idx):
        ...
        out['text_emb'] = torch.load(j_(self.textemb_path, case_id + '.pt'))
        ...
        if self.read_feats:
            ...
            for feat_path in feat_paths:
                ...
                all_features.append(features)
            all_features = torch.from_numpy(np.concatenate(all_features, axis=0))
            if all_features.shape[0] >16384 :
                indices = torch.randint(0, all_features.shape[0], (16384,))
                all_features = all_features[indices]

            out['all_features'] = all_features
        else:
            out['all_features'] = torch.zeros(1,1)
        return out